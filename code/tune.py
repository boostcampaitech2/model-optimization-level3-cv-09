...
RESULT_MODEL_PATH = './optuna/'
n_trial = 1

def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    """Search hyperparam from user-specified search space."""
    epochs = trial.suggest_int("epochs", low=50, high=100, step=30)
    # epochs = 2
    img_size = trial.suggest_categorical("img_size", [112, 224])
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    trainer = TorchTrainer(
        ...
        device=device,
        verbose=1,
        model_path=os.path.join(RESULT_MODEL_PATH, f"automl_{n_trial}.pt"),
    )
    
    trainer.train(train_loader, hyperparams["EPOCHS"], mean_time, val_dataloader=val_loader)
    loss, f1_score, acc_percent = trainer.test(model, test_dataloader=val_loader)
    params_nums = count_model_params(model)

    model_info(model, verbose=True)

    with open(os.path.join(RESULT_MODEL_PATH, f"automl_{n_trial}.yml"), 'w') as outfile:
        yaml.dump(model_config, outfile, default_flow_style=False)
    with open(os.path.join(RESULT_MODEL_PATH, f"automl_{n_trial}_data.yml"), 'w') as outfile:
